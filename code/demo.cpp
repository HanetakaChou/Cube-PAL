	...
	// Assets
	{
		...
		// emissive textrue
		{
			...
			std::vector<PAL_LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST> subresource_memcpy_dests;
			{
				...
				if (device->is_asset_sampled_image_compression_bc_supported())
				{
					emissive_texture_input_stream.init(g_lunarg_dds, sizeof(g_lunarg_dds));
				}
				else
				{
					emissive_texture_input_stream.init(g_lunarg_pvr, sizeof(g_lunarg_pvr));
				}
